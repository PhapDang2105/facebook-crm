...
- [x] QR trỏ về Page **Giọt Nắng** (103549382215599, Page vận hành trong Pancake), ghim bằng `QR_PAGE_ID`/`QR_PAGE_NAME` trên VPS. Không dùng Page Nông Sản Giọt Nắng: token Meta của Page đó đã bị thu hồi ("The user has not authorized application", 190/458), log từ 24/09 không nhận sự kiện Meta nào. (26/09/2026)
- [x] Kiểm chứng 26/09 bằng chẩn đoán `PANCAKE_DEBUG_KEYS`: gói tin webhook Pancake **không có** khoá ref/referral/source nào (chỉ `ads` cho quảng cáo), nên Pancake không chuyển ref của m.me. Lần test 18/09 nhận được ref là qua app Meta của CRM với Page Nông Sản Giọt Nắng (token còn sống lúc đó); Page Giọt Nắng cũng từng nối app Meta (hội thoại và bình luận ngày 14/09 về thẳng app) rồi bị ngắt để chạy Pancake. Đăng ký webhook cấp app vẫn hoạt động, đủ trường; thiếu là cấp Page.
- [x] **Botcake chào khách ngay khi quét (đúng hành vi 18/09, trên Page Giọt Nắng).** Botcake → Công cụ → Messenger Ref URL "Thẻ cảm ơn tmdt-01" (id 2585140): Custom Ref Parameter `tmdt-01`, nội dung = mẫu QR_OFFER với anh/chị, dòng cuối `Mã thẻ: #tmdt-01`. `/q/tmdt-01` đưa khách tới `m.me/103549382215599?ref=tmdt-01`. Tin Botcake dội về CRM qua Pancake → referral BOTCAKE_OPTIN → thống kê. Mỗi lô mới cần thêm một công cụ Botcake (Custom Ref = mã lô) và một luồng tin. (26/09/2026)
- [x] **Đi đường Pancake, không cần nối app Meta.** Chủ shop chỉ đúng: Pancake có "Tạo đường dẫn với nguồn truy cập" (Cài đặt → Công cụ) sinh `m.me/<Page>?ref=<base64url(pancake_utm_source=<tên>)>&text=<tin soạn sẵn>`; đã tạo nguồn `tmdt-01` trong Pancake. CRM dựng đúng dạng đó ở `/q/<mã>`, tin soạn sẵn mang `#<mã>`; khách bấm Gửi là tin về qua webhook Pancake, CRM đọc `#<mã>` thành referral SHORTLINK, đếm thống kê và chào QR_OFFER. Ưu điểm so với ref của Meta: khách gửi một tin thật (vào tệp 365 ngày, mở cửa sổ 24 giờ), không phụ thuộc Meta "không cam kết" gửi ref, không cần nút Bắt đầu. (26/09/2026)
- [ ] Không còn cần: nối Page Giọt Nắng vào app Meta chỉ để nhận referral (`META_REFERRAL_ONLY_PAGES` vẫn giữ trong mã cho Page nào muốn dùng đường Meta). Pancake không chuyển `ref` của m.me về CRM, nên khách quét vào Page Giọt Nắng thì CRM không biết. `META_REFERRAL_ONLY_PAGES=103549382215599` đã đặt trên VPS; chủ tài khoản Facebook bấm Cài đặt → Kênh → Kết nối Facebook, chọn Page Giọt Nắng. CRM chỉ đăng ký `messaging_referrals` + `messaging_postbacks` cho Page này nên hộp thư không nhận tin hai lần.
- [ ] Đặt nút Bắt đầu cho Page Giọt Nắng: sau khi nối, chạy `sudo -u crm node scripts/set-messenger-profile.mjs 103549382215599` trên VPS. Không đặt Ice Breakers (sẽ che nút Bắt đầu; tài liệu không xác nhận referral kèm trong ice breaker).
