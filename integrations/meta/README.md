...
- Không xin `pages_user_gender/locale/timezone`: app loại Kinh doanh dùng Facebook Login for Business, sản phẩm này không hỗ trợ các quyền `pages_user_*` (hộp thoại báo *Invalid Scope*). Giới tính khách lấy theo cách khác — xem mục 6.
...
## 6. Bình luận và quảng cáo

- Webhook `feed` (mục `comment`, hành động `add`/`edited`) đưa bình luận của khách vào cùng hộp thư: mỗi người một hội thoại trên mỗi bài viết (`{page}:comment:{user}:{post}`), có huy hiệu Facebook thay cho Messenger. Like, share, bài mới không được lưu; bình luận bị xóa vẫn giữ trong lịch sử.
- Khách trả lời dưới bình luận của Page thì vào đúng hội thoại cũ; người khác trả lời dưới đó thì mở hội thoại riêng của họ.
- Trả lời trong hội thoại bình luận: mặc định gửi công khai `POST /{comment-id}/comments` dưới bình luận mới nhất của khách; chọn **Nhắn riêng Messenger** thì gọi Send API `POST /{page-id}/messages` với `recipient={comment_id}` (edge `/{comment-id}/private_replies` cũ trả về *Unsupported post request*; Meta chỉ cho một tin riêng cho mỗi bình luận trong 7 ngày, chỉ chữ, không tệp). Tin riêng được ghi vào **hội thoại Messenger** của khách (mở luồng đó nếu chưa có, kế thừa bài viết và giới tính từ luồng bình luận); hội thoại bình luận chỉ giữ bình luận. Bình luận không có giới hạn 24 giờ.
- Bot với bình luận: hội thoại bình luận mở sẵn bot; tin nhắn riêng = mẫu `COMMENT_PRIVATE_REPLY` (câu mở đầu, để trống thì bỏ) + nội dung theo mẫu mô hình chọn, còn dưới bình luận chỉ có một câu theo mẫu `COMMENT_PUBLIC_REPLY` mời kiểm tra Messenger. Cả hai mẫu sửa ở Cài đặt → Thiết lập tin nhắn, nhóm **Bình luận**. Mẫu công khai nên có ≥3 biến thể ngăn bằng `###` (bot chọn ngẫu nhiên một câu — Facebook coi cùng một câu lặp dưới mọi bình luận là spam), dùng `{name}` (tên khách), `{title}` (anh/chị) và `{Dạ|Hi}` (xoay chữ).
- Sau khi trả lời, bot **thích** bình luận và **ẩn** bình luận có số điện thoại (Thiết lập chatbot → Bình luận: thích bật/tắt; ẩn: chỉ khi có SĐT / luôn / không) qua `POST /{comment-id}/likes` và `POST /{comment-id}?is_hidden=true`.
- Bot **bật mặc định cho mọi hội thoại** (inbox lẫn bình luận); nút bot trong hội thoại dùng để tắt khi nhân viên tiếp quản. Không tạo đơn từ bình luận — khách chốt đơn tiếp trong Messenger.
- Hồ sơ khách Messenger chỉ có `name,profile_pic`. Giới tính (để xưng anh/chị) không lấy từ Meta mà theo thứ tự tin cậy: nhân viên bấm Anh/Chị ở bảng Thông tin (`POST customer-panel {type:'gender'}`) > khách tự xưng ở đầu tin nhắn ("chị muốn đặt…") > tên đệm Thị/Văn. Phỏng đoán không bao giờ ghi đè nguồn tin cậy hơn. Mọi mẫu tin nhắn dùng placeholder `{title}`/`{Title}` → anh, chị, hoặc anh/chị khi chưa biết.
