...
- Nhận sự kiện `messaging` của đúng `PANCAKE_PAGE_ID`, cả INBOX lẫn COMMENT; Page khác bỏ qua. Tin trùng mã (Pancake gửi lại khi cập nhật) không ghi hai lần, không gọi bot lần hai.
- **Bình luận** (`conversation.type = COMMENT`, kèm `data.post`): mỗi bình luận thành sự kiện `comment` cùng dạng với webhook feed của Meta (`pancakeCommentEvent`), nên đi vào cùng luồng bình luận của CRM (`{page}:comment:{psid}:{post}`, `applyCommentEvent`): bình luận gốc của Pancake có `parent_id` trỏ về chính nó và `is_parent=true`; trả lời của Page (kể cả nhân viên gõ trong Pancake) nối vào luồng của bình luận cha. Bài viết (`post.message`, ảnh thu nhỏ, link `https://www.facebook.com/{post.id}`) ghi vào `conversation.post` để bot đoán sản phẩm. Bot xử lý như bình luận Meta: nhắn riêng bằng `POST …/conversations/{comment_conversation}/messages` `{action:"private_replies", post_id, message_id, from_id, message}` (ghi vào hộp thư Messenger của khách, hộp thư đó nhận `pancakeConversationId = {page}_{psid}` để các tin sau đi thẳng), rồi trả lời công khai bằng `{action:"reply_comment", message_id, message}`. Pancake không có API thích/ẩn bình luận nên `moderateComment` bỏ qua với luồng Pancake. Nhân viên trả lời dưới bình luận trong Pancake thì bot tạm dừng luồng đó như với inbox.
- **Quảng cáo**: hội thoại từ quảng cáo có `ads[] = {ad_id, post_id, inserted_at}` (cả trong danh sách hội thoại lẫn webhook khi Pancake gửi kèm). CRM ghi `conversation.referral = {source:"ADS", adId, postId, adTitle, photoUrl}` như Meta (chỉ khi mã quảng cáo đổi), rồi `enrichPancakeAdContext` tra tên và ảnh quảng cáo bằng `GET /v1/pages/{page}/ads?ad_ids=` (nhớ 24 giờ, gọi ngay trước khi đưa bot) và tìm nội dung bài quảng cáo bằng `GET /v1/pages/{page}/posts?since&until&page_number&page_size` lùi dần từng tháng (Pancake không có API đọc bài theo id; tìm nền, nhớ kết quả) rồi ghi `conversation.post.message`. `resolveConversationProduct` đọc `referral.adTitle` và `post.message` nên bot biết khách đang hỏi sản phẩm nào; Khách hàng lọc được nguồn "ads".
- Đồng bộ định kỳ kéo thêm 30 luồng bình luận mới nhất (`type=COMMENT`, kèm `post` từ API tin) vào hộp thư, không đưa bot.
...
- Ảnh: ảnh sản phẩm của mẫu tin và tệp nhân viên đính kèm từ CRM được tải lên bằng `POST /v1/pages/{page_id}/upload_contents` (multipart `file`) rồi gửi bằng `content_ids` (không gửi cùng `message`); mỗi lần gửi tải ảnh lên mới (mã nội dung chỉ dùng được một lần, bị từ chối nếu dùng lại). Pancake chỉ nhận tệp tới **500 KB** (trả 200 kèm `success:false`, "File size should not exceed 500KB"), nên ảnh lớn hơn được thu nhỏ (cạnh dài 1080px) và nén JPEG bằng `sharp` trước khi tải; tệp không phải ảnh phải tự nhỏ hơn 500 KB. Thẻ receipt của Messenger không gửi được qua Pancake, nên khi chốt đơn qua Pancake phiếu xác nhận được vẽ thành ảnh PNG (`app/order-receipt-image.mjs`, bố cục y thẻ "Xác nhận đơn đặt hàng" trong hộp thư: ảnh sản phẩm, số lượng, đơn giá, giờ đặt, SĐT, thanh toán, giao đến, giá trị đơn; chữ vẽ bằng sharp với font Roboto của dự án) và gửi như ảnh đính kèm sau tin ORDER_CONFIRMATION. Ảnh/video khách gửi (và bản dội lại của ảnh Page gửi) có `attachments[].url` trên CDN Pancake, hộp thư hiện thẳng.
