...
- Ảnh: ảnh sản phẩm của mẫu tin và tệp nhân viên đính kèm từ CRM được tải lên bằng `POST /v1/pages/{page_id}/upload_contents` (multipart `file`) rồi gửi bằng `content_ids` (không gửi cùng `message`); mã nội dung của ảnh sản phẩm được nhớ 12 giờ. Receipt dùng bản chữ. Ảnh/video khách gửi (và bản dội lại của ảnh Page gửi) có `attachments[].url` trên CDN Pancake, hộp thư hiện thẳng.
