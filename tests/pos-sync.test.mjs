...
test('đồng bộ: đơn webhook đã có thì gộp và ghi mã POS; đơn POS mới (kể cả bỏ dở) thì tạo, lần sau bỏ qua', async () => {
  ...
  const pages = [posOrder(), posOrder({ id: 51938, bill_full_name: 'Nguyễn tú anh', bill_phone_number: '0368419478', is_abandoned_order: true, status: 0, inserted_at: '2026-09-15T19:08:42.000000', items: [], total_price: 0, shipping_address: { full_address: 'Bắc Kạn', province_name: 'Bắc Kạn' }, link: 'https://granola.giotnang.vn/?utm_campaign=camp1' }), { id: 1, order_sources_name: 'Facebook', link: '' }];
  ...
  assert.equal(first.landing, 2);
  assert.equal(first.created, 1, 'đơn bỏ dở của Nguyễn tú anh được tạo');
